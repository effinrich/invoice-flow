import { RefreshCw, Pause, Play, Trash2, Edit2, Copy, Link2, CreditCard, RotateCcw, Plus } from 'lucide-react'
    ...
    <div className="rounded-2xl border p-5 transition-shadow hover:shadow-md" style={{ background: isPaused ? '#faf9f7' : '#fff', borderColor: isPaused ? '#e8e0d8' : '#e2d8d0', opacity: isPaused ? 0.8 : 1 }}>
      <div className="flex items-start justify-between gap-3 mb-4">
        <div className="flex items-center gap-3 min-w-0">
          {item.logoUrl ? (
            <img src={item.logoUrl} alt="" className="w-10 h-10 rounded-xl object-contain border shrink-0" style={{ borderColor: '#f0ece8', background: '#fff' }} />
          ) : (
          ...
          <div className="min-w-0">
            ...
            <div className="flex items-center gap-2 mt-0.5 flex-wrap">
              <span className="inline-flex items-center gap-1 text-xs font-medium" style={{ color: '#9c8572' }}><RotateCcw size={10} />{FREQUENCY_LABELS[item.frequency]}</span>
      ...
      {lastInvoiceId && (
        <div className="flex items-center justify-between gap-2 px-3 py-2 rounded-xl mb-3 border" style={{ background: 'hsl(16 95% 97%)', borderColor: 'hsl(16 60% 88%)' }}>
          <div className="flex items-center gap-2 min-w-0"><Link2 size={12} style={{ color: 'hsl(16 95% 52%)', flexShrink: 0 }} /><span className="text-xs font-medium truncate" style={{ color: 'hsl(16 80% 35%)' }}>Client portal ready</span></div>
...
function EmptySchedules({ onNew }: { onNew: () => void }) {
  return (
    <div className="flex flex-col items-center justify-center py-16 text-center">
      ...
      <h3 className="text-lg font-bold mb-1" style={{ color: '#1a1208' }}>No recurring schedules yet</h3>
      <p className="text-sm mb-6 max-w-sm" style={{ color: '#9c8572' }}>Set up automated billing for retainers, subscriptions, and monthly service agreements.</p>
...
export function SchedulesTab({ items, lastGeneratedMap, onGenerate, onEdit, onToggle, onDelete, onCopyPortalLink, onViewPortal, onNew }: SchedulesTabProps) {
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      {items.map(item => (
        <RecurringCard
          ...
          onCopyPortalLink={() => { const lid = lastGeneratedMap[item.id]; if (lid) onCopyPortalLink(lid) }}
