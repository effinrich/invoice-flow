...
async function generateAndSend(item: RecurringInvoice, userId: string, portalBaseUrl: string) {
  ...
  const { error: insertError } = await supabase.from("generated_invoices").insert({
    id: invoiceId,
    user_id: userId,
