...
export function useRecurringInvoices(userId: string | null) {
  ...
  const createMutation = useMutation({
    mutationFn: async (data: Omit<RecurringInvoice, 'id' | 'createdAt' | 'updatedAt' | 'invoiceCount' | 'lastGeneratedAt'>) => {
      const now = new Date().toISOString()
      const id = `rec_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`
      await blink.db.recurringInvoices.create({
        id,
  ...
  const updateMutation = useMutation({
    mutationFn: async ({ id, ...data }: Partial<RecurringInvoice> & { id: string }) => {
      ...
      await blink.db.recurringInvoices.update(id, row)
    },
    onSuccess: invalidate,
  })
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      await blink.db.recurringInvoices.delete(id)
    },
    onSuccess: invalidate,
  })
